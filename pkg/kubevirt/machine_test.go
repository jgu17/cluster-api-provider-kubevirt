...
var _ = Describe("Without KubeVirt VM running", func() {
	...
	It("NewMachine should have client and machineContext set, but vmiInstance equal nil", func() {
		externalMachine, err := defaultTestMachine(machineContext, namespace, fakeClient, fakeVMCommandExecutor, []byte{}, nil, networkDataSecret)
...
var _ = Describe("With KubeVirt VM running", func() {
	...
	It("NewMachine should have all client, machineContext and vmiInstance NOT nil", func() {
		externalMachine, err := defaultTestMachine(machineContext, namespace, fakeClient, fakeVMCommandExecutor, []byte(sshKey), nil, networkDataSecret)
		Expect(err).NotTo(HaveOccurred())
...
func defaultTestMachine(ctx *context.MachineContext, namespace string, client client.Client, vmExecutor FakeVMCommandExecutor, sshPubKey []byte, serviceAccountSeceret *corev1.Secret, networkDataSecret *corev1.Secret) (*Machine, error) {

	machine, err := NewMachine(ctx, client, namespace, &ssh.ClusterNodeSshKeys{PublicKey: sshPubKey}, serviceAccountSeceret, networkDataSecret)
